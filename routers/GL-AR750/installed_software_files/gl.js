		...
		success : function(result){
			...
			for(var i=0;i<wifis.length;i++){
				...
				if(i>=1 && !document.getElementById("div-wifi-"+i)){
					...
					$("#div-wifi-"+i).off('click').on('click',function(){
						if(mobile){
					...
					$("#img-wifi-"+i).off('click').on('click',function(e){
						popWifiSetting(i);
					});
				}
			...
			$("[id^='wifi_status']").off('click').on('click',function(e){
				e.stopPropagation();
			});
			$("[id^='wifi_status']").off('change').on('change',function(e){
				e.stopPropagation();
				stopChecker();
				var status;
				if($(this).is(':checked'))
	...
	$.ajax({
		...
		success : function(result){
			...
			if(disks.length+videos.length==0){
				...
				flag_devices=0;
				$("#device0").attr("onclick", "javascript: $('#div_main').html('<h2>No storage device detected</h2>')");
				return;
			}
			var div_device;
			for(i=0, index=0;i<disks.length; i++, index++){
				...
				if(mobile && index>0){
					$(div_device).off('click').on('click',function(e){
						$(".mobile-main").toggleClass("hidden");
						$("#div-main-holder").toggleClass("hidden");
					});
				}
		...
		success: function(result){
			if(result.success){
				...
				for (var i=0; i<contents.length; i++){
					...
					has_share="none";
					console.log(file)
					if(file_share_id.length > 0) {
						has_share="block"
					};
					//  ["d", "16KB", "2017-05-15 15:54:48", "$RECYCLE.BIN", "", "0"] 文件目录带D
					if(file[0]=='d'){
						...
						html += folder_str;
					}else{	//	["-", "20KB", "2017-11-07 07:01:58", "index.png", "", "0"] 最子级目录为空格
						file_str="<tr onmouseover='displaylink(this)' onmouseout='hidelink(this)'>"+
									"<td><a href=\"javascript:downloadContent(&quot;"+unescape(encodeURIComponent(path))+"/"+unescape(encodeURIComponent(file_name))+"&quot;)\"><image src='/images/file.png'>"+file_name+"</a></td>"+
									"<td>"+file[1]+"</td>"+
									"<td>"+file[2]+"</td>"+
									"<td>"+
										"<a style='display:"+has_share+";cursor:pointer' onclick=\"showLinkInfo(&quot;"+path+"/"+file_name+"&quot;,this)\"><img src='/images/link.png'></a>"+
									"</td>"+
								"</tr>";
						html += file_str;
					}
				...
				html+="</table>"
				$("#storage_content").html(html);
				$(".mobile-main").addClass("hidden");
				$("#div-main-holder").removeClass("hidden");
			}else{
				window.location="/html/login";
			}
		}
...
function fillLinkInfo(file,a_parent){
	...
	$("#button_link").off('click').on('click',function linkContent(){
		$.ajax({
			type : 'GET',
			dataType: 'json',
	...
	$("#button_unlink").off('click').on('click',function removeLink(){
		$.ajax({
			type : 'GET',
			dataType: 'json',
		...
		success : function(result){
			...
			if(devices && devices.length==0){
				...
				for(var i=0;i<devices.length;i++){
					device=devices[i];
					var desc="";
					if(device.includes("cdc-wdm"))desc=" (qmi)";
					s="<option value='"+device+"'>"+ device+ desc+"</option>";
					$("#3g-device").append(s);
				}
			}
	...
	$.ajax({
		...
		success : function(result){
			...
			for(var i=0;i<stations.length;i++){
				...
				if(stations[i].Radar=="yes"){
					stations[i].SSID=stations[i].SSID+" (DFS),not supported";
					var s="<option disabled='disabled' style='word-break:break-all' value='"+stations[i].SSID+"' data-image='"+img+"'>"+ stations[i].SSID.replace(/\s/g, '&nbsp;');+"</option>";
				}
				else{
					var s="<option value='"+stations[i].SSID+"' data-image='"+img+"'>"+ stations[i].SSID.replace(/\s/g, '&nbsp;');+"</option>";
				}
				$("#wan-ssid").append(s);
			}
			$("#wan-ssid").msDropDown();
